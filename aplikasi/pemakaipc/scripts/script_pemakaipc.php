...
<script>
    ...
    $(document).ready(function() {
        ...
        function populateDropdown(column, selectId) {
            ...
            let url = 'aplikasi/pemakaipc/actions/get_filter_data.php';
        ...
        function loadData(page = 1, forceUpdate = false) {
            ...
            let url = isFiltered 
                ? 'aplikasi/pemakaipc/modals/filter_data.php'
                : 'aplikasi/pemakaipc/modals/fetch_pemakaipc.php';
                ...
                success: function(response) {
                    ...
                    if (!response || !response.data || response.data.length === 0) {
                        ...
                        $.each(response.data, function(index, row) {
                            ...
                            html += '<td class="center"><form action="user.php?menu=fupdate_pemakaipcNew" method="post"><input type="hidden" name="nomor" value="' + (row.nomor || '') + '" /><input type="hidden" name="id" value="' + (row.id || '') + '" /><button class="btn btn-primary" type="submit">Perawatan</button></form></td>';
                            html += '<td class="center"><form action="user.php?menu=updaterpemakaipc" method="post"><input type="hidden" name="id" value="' + (row.id || '') + '" /><input type="hidden" name="nomor" value="' + (row.nomor || '') + '" /><button class="btn btn-primary" type="submit">Update</button></form></td>';
                            html += '<td class="center"><form action="aplikasi/pemakaipc/actions/stc_deletdatas.php" method="post"><input type="hidden" name="id" value="' + (row.id || '') + '" /><input type="hidden" name="nomor" value="' + (row.nomor || '') + '" /><button class="btn btn-danger" type="submit">X</button></form></td>';
                            html += "</tr>";
                        });
                    }
                    $("#dataBody").html(html);
                    ...
                    $.ajax({
                        url: 'aplikasi/pemakaipc/actions/save_page.php',
                        method: 'POST',
...
$('#exportPdfBtn').on('click', function() {
    ...
    let form = $('<form></form>').attr('action', 'aplikasi/pemakaipc/actions/exportpdf_data.php').attr('method', 'POST').attr('target', '_blank');
    ...
    alert('Export PDF selesai, file telah diunduh.');
       ...
       // Fungsi untuk tombol Export Excel
        $('#exportExcelBtn').on('click', function() {
            ...
            let form = $('<form></form>').attr('action', 'aplikasi/pemakaipc/actions/exportexcel_data.php').attr('method', 'POST').attr('target', '_blank');
            $.each(exportFilterData, function(key, value) {
                form.append($('<input>').attr('type', 'hidden').attr('name', key).attr('value', value));
            });
        ...
        $('#dataBody').on('submit', 'form[action="aplikasi/pemakaipc/actions/stc_deletdatas.php"]', function(e) {
            e.preventDefault();
