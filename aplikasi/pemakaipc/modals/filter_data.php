...
if ($totalResult === false) {
    error_log("Total query gagal: " . print_r(sqlsrv_errors(), true));
    echo json_encode(['error' => 'Total query gagal: ' . print_r(sqlsrv_errors(), true)]);
    exit;
}
...
while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
    // Konversi kolom tgl_perawatan (dan kolom DateTime lainnya) ke string
    if (isset($row['tgl_perawatan']) && $row['tgl_perawatan'] instanceof DateTime) {
        $row['tgl_perawatan'] = $row['tgl_perawatan']->format('Y-m-d');
    }

    // Debugging: Log data yang akan dikirim untuk memastikan format
    error_log("Data baris: " . print_r($row, true));
