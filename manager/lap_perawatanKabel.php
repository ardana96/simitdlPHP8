...
{
    $nb = rand(3, 10); // Pastikan variabel diinisialisasi
    $w = '';
    for ($i = 1; $i <= $nb; $i++) {
...
{
    $nb = rand(1, 10); // Pastikan variabel diinisialisasi
    $s = '';
    for ($i = 1; $i <= $nb; $i++) {
        $s .= GenerateWord() . ' ';
    }
...
while ($database = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    // Pastikan nilai DateTime dikonversi ke string
    ...
    $data = array(
        array(
            ...
            $database['item1'], $database['treated_by'], $database['approve_by'], ''
        )
    );
    foreach ($data as $row) {
